...
public class ADecisionTree extends Classifier {
    ...
    private double predict_with_decision_tree(double[] features, ATreeNode node) {
    ...
    private ATreeNode build_decision_tree(int[] set, int[] attr_index) {
        ...
        //寻找最优切割属性
        ASplitData split_info = attribute_selection(node);
        ...
        //递归建立子节点
        node.childrenNodes = new ATreeNode[split_info.split_sets.length];
    ...
    private ASplitData attribute_selection(ATreeNode node) {
        ...
        if (_isClassification) {
            for (int attribute : node.attr_index) {
                try {
                    ABundleData gain_ratio_info = gain_ratio_use_attribute(node.set, attribute); //分割错误会抛出分割异常
            ...
            for (int attribute : node.attr_index) {
                try {
                    ABundleData mse_info = mse_use_attribute(node.set, attribute);
